...
var full = [];
    ...
    var array = [number[0],number[1],number[2]];
        ...
        if (array.includes(itemm)) {
            ...
            if (i == 1) {
                var mon = parseInt(args[1]) + 300;  
                    await Currencies.increaseMoney(event.senderID, mon); console.log("s1")
                        return api.sendMessage({body:`Kết Quả : ${full.join("|")}\n[ HNT ] => Được ${mon} Đô,Vì Có 1 ${icon}!`,attachment: listimg},event.threadID, event.messageID);
            }
            else if (i == 2) {
                var mon = parseInt(args[1]) * 2; 
                    await Currencies.increaseMoney(event.senderID, mon); console.log("s2")
                        return api.sendMessage({body:`Kết Quả : ${full.join("|")}\n[ HNT ] => Được ${mon} Đô,Vì Có 2 ${icon}!`,attachment: listimg},event.threadID, event.messageID);
            }
            else if (i == 3) {
                var mon = parseInt(args[1]) * 3; 
                    await Currencies.increaseMoney(event.senderID, mon); console.log('s3')
                        return api.sendMessage({body:`Kết Quả : ${full.join("|")}\n[ HNT ] => Được ${mon} Đô,Vì Có 3 ${icon}!`,attachment: listimg},event.threadID, event.messageID);
            }
            ...
            return api.sendMessage({body:`Kết Quả : ${full.join("|")}\n[ HNT ] => Trừ ${args[1]} Đô,Vì Có 0 ${icon}!`,attachment: listimg},event.threadID, event.messageID);
        }
            } ,event.messageID);
    };
